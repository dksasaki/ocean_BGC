   ...
   subroutine generic_CBED_sediments_update_from_source(cobalt_tracer_list, cobalt, phyto, ilb, jlb, mask_coast, &
      ...
      do j = jsc, jec; do i = isc, iec
            if (grid_kmt(i,j) .gt. 0) then
               ...
               cobalt%b_o2(i,j)  = b_o2(i,j) + 0.1*b_o2(i,j)   ! + max(0.0, - (cbed%odu_flux(i,j)))  ! Add the ODU flux as added oxygen demand by the sediment because released ODU will be consummed in the bottom water. 
